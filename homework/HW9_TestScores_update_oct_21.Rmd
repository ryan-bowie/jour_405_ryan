...
Yes, the scores have improved. The scores from all classrooms are higher than the prior mean of 72.6.
...
#Why is there an 'NA' above where the standard deviation of 1.8 should be?

The average scores went up across the board and since the standard deviation is 1.8, that means that more students performed better with less variance between the different scores. One question I would ask from a reporinting standpoint would be "Why did this new reading program work?" or "What was different from this reading plan and the one previously in place?" 
...
my_fill_color <- "navyblue" # Replace with a color name like "royalblue", "darkgreen", etc.
...
Perhaps "Classroom Reading Scores Compared to Previous District Benchmark."
...
**Hypotheses:**
Null: The new reading program has not increased test scores
Alternative: The new reading program has increased test scores
...
The p-value tells me how likely it would be that the test scores would be higher if the average score was still 72.6. In a non-technical way, I would say it would be very rare for the scores to randomly increase without the implementation of the new reading program.
...
Headline: Minor Tweaks Show Major Improvement to Standardized Test Scores

Lead: A new reading program implemented throughout Prince George's County Public Schools yielded an increase by over four percentage points to standardized testing scores throughout the district, according to the district superintendent.
...
Advancement rates and vocabulary literacy are two metrics that could assist in assessing reading performance. The more students that advance from one grade to another will show that the program is working. Also, the more complex vocabulary that is used in writing assignments will also prove that the children are not only understanding, but using the words more frequently, proving reading comprehension as well.
