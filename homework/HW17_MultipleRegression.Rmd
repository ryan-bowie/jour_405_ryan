...
All have a significant linear relationship with the exception of population. Income, median_age, pct_bachelors_higher and pct_white all have strong positive relationships while pct_unemployed is a strong negative relationship. Population doesn't dictate whether people vote or not, hence the lack of a true linear relationship.
...
Overall, the model is useful. 85% of turnout variability is explained by the model, the f-statistic is large, indicating the results of the test aren't random, but statistically significant. The residual standard error shows that the test has a margin of error of roughly 2 percentage points.

Income, median age and percent white are all statistically significant with p-values lower than 0.05. Population is insignificant, but surprisingly, education and unemployed are also statistically insignificant.
...
# Model 3
model3 <- lm(turnout ~ income + pct_bachelors_higher + pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ income + pct_white, data = md_turnout)
summary(model4)
...
Using this analysis, I can show how the models vary in accuracy and how it relates to the model that takes all variables into account. Model 2 represents the three strongest positive variables (income, median_age, pct_white), Model 3 is all variables except the weakest one (population) and Model 4 is the strongest two variables (income, pct_white). With this analysis, I can prove that population has no real impact on whether people vote or not.
